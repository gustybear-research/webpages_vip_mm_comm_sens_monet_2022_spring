...
To evaluate the effectiveness of MIMO in an optimal arrangement of transmitters and receivers, we will perform several experiments involving the use of a 2x2 channel array with transmitter and receiver antennas. The transmitter and receiver will be powered by 2 computers, with the transmitter using a phase array to continuously beam-form a signal across the distance from transmitter to receiver. A Windows PC will be used to move the transmitter and receiver antenna, as well as collect the CSI, or Channel State Information that allows us to calculate the signal conditions from the transmitter to receiver, which will tell us the optimal positions for the antennas and the reflector surface. 

The mmWave channel consists of a mmWave beamforming kit by TMYTEK which includes the BBox Lite and the UD box, a mmWave frequency converter, the Ettus USRP N210 and the Intel 5300 commercial off-the-shelf, COTS, Wi-fi network interface card, NIC. The UD box frequency converter interfaces with the baseband USRP or COTS WIFI NIC devices and these baseband devices are controlled using LabVIEW. The low frequency devices can support up to 80Mhz bandwidth. For this research, the USRP N210 will be generating a signal at 2.4 GHz of which the signal will be converted to 28GHz through the frequency converter which is connected to two phased array radio heads to receive the signal. 
